...
public:
  ...
  publish(std::unique_ptr<T, PublishedTypeDeleter> msg)
  {
    ...
    if constexpr (rclcpp::detail::is_protobuf_message<PublishedType>::value) {
      if (intra_process_is_enabled_) {
        throw std::runtime_error(
          "publishing protobuf messages with intra-process enabled is not supported");
      }
      ROSMessageType serialized;
      rclcpp::TypeAdapter<MessageT>::convert_to_ros_message(*msg, serialized);
      return this->publish(serialized);
    }
#endif
    // Avoid allocating when not using intra process.
    if (!intra_process_is_enabled_) {
  ...
  /**
   * Enabled when MessageT is a protobuf message type and no TypeAdapter is used.
   * Serialization is handled by the protobuf FastRTPS typesupport.
   */
  template<typename T>
  typename std::enable_if_t<
    !rclcpp::TypeAdapter<MessageT>::is_specialized::value &&
    rclcpp::detail::is_protobuf_message<T>::value &&
    std::is_same<T, PublishedType>::value
  >
  publish(const T & msg)
  {
