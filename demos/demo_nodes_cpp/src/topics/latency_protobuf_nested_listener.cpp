...
class LatencyProtobufNestedListener : public rclcpp::Node
{
public:
  explicit LatencyProtobufNestedListener(const rclcpp::NodeOptions & options)
  : rclcpp::Node("latency_protobuf_nested_listener", options)
  {
    ...
    sub_ = this->create_subscription<demo_nodes_cpp::proto::Nested>(
      topic_, rclcpp::QoS(10),
      [this](const demo_nodes_cpp::proto::Nested & msg) { this->on_msg(msg); });
  }

private:
  void on_msg(const demo_nodes_cpp::proto::Nested & msg)
  {
    ...
    if (samples_us_.size() >= total_msgs_) {
      ...
      RCLCPP_INFO(
        this->get_logger(),
        "PROTO nested: n=%zu mean=%.2fus p50=%.2fus p95=%.2fus p99=%.2fus max=%.2fus",
  ...
  rclcpp::Subscription<demo_nodes_cpp::proto::Nested>::SharedPtr sub_;
};
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<demo_nodes_cpp::LatencyProtobufNestedListener>(rclcpp::NodeOptions{}));
  rclcpp::shutdown();
  return 0;
}
