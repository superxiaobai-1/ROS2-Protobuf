...
class LatencyProtobufBigListener : public rclcpp::Node
{
public:
  explicit LatencyProtobufBigListener(const rclcpp::NodeOptions & options)
  : rclcpp::Node("latency_protobuf_big_listener", options)
  {
    ...
    sub_ = this->create_subscription<demo_nodes_cpp::proto::BigBytes>(
      topic_, rclcpp::QoS(10),
      [this](const demo_nodes_cpp::proto::BigBytes & msg) { this->on_msg(msg); });
  }

private:
  void on_msg(const demo_nodes_cpp::proto::BigBytes & msg)
  {
    ...
    if (samples_us_.size() >= total_msgs_) {
      ...
      RCLCPP_INFO(
        this->get_logger(),
        "PROTO big: n=%zu mean=%.2fus p50=%.2fus p95=%.2fus p99=%.2fus max=%.2fus",
  ...
  rclcpp::Subscription<demo_nodes_cpp::proto::BigBytes>::SharedPtr sub_;
};
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<demo_nodes_cpp::LatencyProtobufBigListener>(rclcpp::NodeOptions{}));
  rclcpp::shutdown();
  return 0;
}
