...
public:
  ...
  : rclcpp::Node("latency_protobuf_nested_listener", options)
  {
    ...
    sub_ = this->create_subscription<example::proto::Nested>(
      topic_, rclcpp::QoS(10),
      [this](const example::proto::Nested & msg) { this->on_msg(msg); });
  }

private:
  void on_msg(const example::proto::Nested & msg)
  {
  ...
  rclcpp::Subscription<example::proto::Nested>::SharedPtr sub_;
};
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<example::LatencyProtobufNestedListener>(rclcpp::NodeOptions{}));
  rclcpp::shutdown();
  return 0;
}
